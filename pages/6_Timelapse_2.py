...
def calculate_years_with_frequency(start_year, end_year, frequency_years):
    """
    Calculate years to include in timelapse based on frequency interval.
    
    Args:
        start_year (int): Starting year
        end_year (int): Ending year
        frequency_years (int): Interval between years (1-20)
    ...
    while current_year <= end_year:
        ...
        current_year += frequency_years
    
    # Ensure end year is included if it doesn't align with frequency
    if years and years[-1] != end_year and end_year > years[-1]:
...
def create_landsat_rgb_timelapse(roi, out_gif, start_date, end_date, vis_params, frequency_years=1, **kwargs):
    """Creates a true-color Landsat timelapse using annual composites with flexible frequency."""
    start_year = int(start_date.split('-')[0])
    end_year = int(end_date.split('-')[0])
    
    # Calculate years based on frequency
    selected_years = calculate_years_with_frequency(start_year, end_year, frequency_years)
    years = ee.List(selected_years)
...
# Enhanced LST timelapse function with frequency support
def create_optimized_landsat_lst_timelapse(
    roi, out_gif, start_date, end_date, frequency_years=1, **kwargs
):
    """Creates a timelapse of Landsat Land Surface Temperature using efficient annual composites with flexible frequency."""
    start_year = int(start_date.split('-')[0])
    end_year = int(end_date.split('-')[0])
    ...
    def create_yearly_composite(year):
        # Call the helper function for each year
...
def create_landsat_index_timelapse(
    roi, out_gif, index_type, start_year, end_year, frequency_years=1, **kwargs
):
    """Creates a timelapse for Landsat indices with flexible frequency support."""
    ...
    if index_type in ['UTFVI', 'UHS']:
        st.info(f"Info: {index_type} calculation is complex and will be processed year-by-year (every {frequency_years} year{'s' if frequency_years > 1 else ''}), which may be slow. Please be patient.")
    ...
    with row1_col2:
        ...
        with st.form(form_key):
            with st.expander("Customize timelapse", expanded=True):
                ...
                st.markdown("### Temporal Frequency Settings")
                
                # Calculate total time span
                total_years = (end_date.year - start_date.year) + 1
                
                # Display info about time span
                st.info(f"**Time span:** {total_years} year{'s' if total_years != 1 else ''} ({start_date.year}-{end_date.year})")
                
                # Frequency slider with dynamic range
                max_frequency = min(20, total_years) if total_years > 1 else 1
                frequency_years = st.slider(
                    "Temporal frequency (years between frames):", 
                    min_value=1, 
                    max_value=max_frequency, 
                    value=1,
                    help=f"Select interval between years. With current settings, this will create approximately {len(calculate_years_with_frequency(start_date.year, end_date.year, 1))} frames at 1-year intervals."
                )
                
                # Calculate and display frame information
                selected_years = calculate_years_with_frequency(start_date.year, end_date.year, frequency_years)
                estimated_frames = len(selected_years)
                
                # Display preview of selected years
                with st.expander(f"Preview: {estimated_frames} frames will be generated", expanded=False):
                    if estimated_frames <= 20:
                        st.write("**Years to be processed:**")
                        st.write(", ".join(map(str, selected_years)))
                    else:
                        st.write(f"**Years to be processed:** {selected_years[0]}, {selected_years[1]}, ..., {selected_years[-2]}, {selected_years[-1]} (showing first/last)")
                    
                    # Performance warning for large datasets
                    if estimated_frames > 30:
                        st.warning(f"⚠️ Processing {estimated_frames} frames may take significant time. Consider increasing the frequency interval for faster processing.")
                    elif estimated_frames > 15:
                        st.info(f"ℹ️ Processing {estimated_frames} frames. This may take a few minutes.")
            ...
            if submitted:
                ...
                # Validate frequency
                if frequency_years > total_years:
                    st.warning(f"Frequency ({frequency_years} years) is larger than total time span ({total_years} years). Using 1-year frequency instead.")
                    frequency_years = 1
                ...
                empty_text.text(f"Computing {estimated_frames} frames (every {frequency_years} year{'s' if frequency_years > 1 else ''})... Please wait...")
                ...
                try:
                    if collection == "Landsat TM-ETM-OLI Surface Reflectance":
                        ...
                        status_text.text("Processing RGB composite...")
                        ...
                        status_text.text("Processing Land Surface Temperature...")
                        create_optimized_landsat_lst_timelapse(
                            roi=roi, out_gif=out_gif, start_date=start_date.strftime("%Y-%m-%d"), 
                            end_date=end_date.strftime("%Y-%m-%d"),
                            frequency_years=frequency_years,
                            palette=custom_palette, frames_per_second=speed, title=title,
                            font_size=font_size, font_color=font_color, add_progress_bar=add_progress_bar, 
                            progress_bar_color=progress_bar_color, mp4=mp4,
                            overlay_data=overlay_data, overlay_color=overlay_color, 
                            overlay_width=overlay_width, overlay_opacity=overlay_opacity
                        )
                        ...
                        status_text.text(f"Processing {index_type}...")
                        create_landsat_index_timelapse(
                            roi=roi, out_gif=out_gif, index_type=index_type, 
                        ...
                        status_text.text("Processing custom collection...")
                    ...
                    status_text.text("Finalizing timelapse...")
                    
                    if os.path.exists(out_gif):
                        geemap.reduce_gif_size(out_gif)
                        empty_text.text("✅ Timelapse completed! Right-click the GIF to save it to your computer.")
                        empty_image.image(out_gif)
                        
                        # Display summary information
                        st.success(f"""
                        **Timelapse Summary:**
                        - **Frames generated:** {estimated_frames}
                        - **Years processed:** {', '.join(map(str, selected_years[:3]))}{'...' if len(selected_years) > 3 else ''}
                        - **Frequency:** Every {frequency_years} year{'s' if frequency_years > 1 else ''}
                        - **Time span:** {start_date.year}-{end_date.year}
                        """)
                    ...
                    empty_text.error(f"❌ An error occurred: {e}. Try reducing the ROI size, timespan, or increasing the frequency interval.")
                    st.exception(e)
